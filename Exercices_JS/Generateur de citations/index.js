//todo : Réalisation d'un générateur de citations aléatoires en JS. Design et CSS via bootstrap.
...
let dataIndex = citations.length; // variable qui s'ajuste automatiquement à la longueur du tableau de citations.
let newCitation; // variable globale qui va contenir la citation aléatoire.
let author; // variable globale qui va contenir l'auteur de la citation aléatoire.

// fonction qui génère un index aléatoire en fonction de la longueur du tableau de citations.
function randomIndex(){
    let newIndex = Math.floor(Math.random() * dataIndex);
    return newIndex;
};

// fonction qui génère une citation aléatoire depuis le tableau de citations, en fonction de l'index aléatoire.
function randomCitation() {
    indexRdm = randomIndex(); 
    console.log(indexRdm);   
    newCitation = citations[indexRdm][0];
    author = citations[indexRdm][1];
    // console.log(newCitation);
    // console.log(author);
    return newCitation && author;
};

// fonction qui affiche la citation aléatoire et son auteur dans le DOM, en lieux et place de la citation actuelle.
function displayCitation(){
    randomCitation();
    let displayCitation = document.querySelector('#citation');
    displayCitation.textContent = newCitation;
    let displayAuthor = document.querySelector('#auteur');
    displayAuthor.textContent = author;    
};

// On cible le bouton "nouvelle citation" et on lui ajoute un écouteur d'événement qui va déclencher la fonction displayCitation à chaque clic.
document.querySelector("#nouveau").addEventListener('click', displayCitation);
